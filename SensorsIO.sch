EESchema Schematic File Version 4
LIBS:TPDSI_1-cache
EELAYER 29 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 3 4
Title "Kit Development for MCU PIC"
Date "2019-06-04"
Rev "1.3"
Comp "Autor: Julian Bustamante Narvaez"
Comment1 "Rev : Jacobo Salvador"
Comment2 "DCI-CESE 7Cohorte UBA."
Comment3 ""
Comment4 ""
$EndDescr
Wire Wire Line
	5800 3900 5800 3800
$Comp
L Device:R R?
U 1 1 5CE24118
P 5800 3650
AR Path="/5CCF7078/5CE24118" Ref="R?"  Part="1" 
AR Path="/5CCF6F7E/5CE24118" Ref="R7"  Part="1" 
AR Path="/5D002D0B/5CE24118" Ref="R1"  Part="1" 
AR Path="/5D086332/5CE24118" Ref="R?"  Part="1" 
F 0 "R1" H 5870 3696 50  0000 L CNN
F 1 "330" H 5870 3605 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 5730 3650 50  0001 C CNN
F 3 "https://www.rohm.com/datasheet/KTR03EZPF/ktr-e" H 5800 3650 50  0001 C CNN
F 4 "RHM330AZCT-ND" H 5800 3650 50  0001 C CNN " Part Number"
	1    5800 3650
	1    0    0    -1  
$EndComp
$Comp
L Device:LED D?
U 1 1 5CE2411E
P 5800 4050
AR Path="/5CCF7078/5CE2411E" Ref="D?"  Part="1" 
AR Path="/5CCF6F7E/5CE2411E" Ref="D4"  Part="1" 
AR Path="/5D002D0B/5CE2411E" Ref="D4"  Part="1" 
AR Path="/5D086332/5CE2411E" Ref="D?"  Part="1" 
F 0 "D4" V 5839 3933 50  0000 R CNN
F 1 "LED" V 5748 3933 50  0000 R CNN
F 2 "LED_SMD:LED_0603_1608Metric_Castellated" H 5800 4050 50  0001 C CNN
F 3 "http://optoelectronics.liteon.com/upload/download/DS22-2000-228/LTST-C191KGKT.PDF" H 5800 4050 50  0001 C CNN
F 4 "160-1447-1-ND" V 5800 4050 50  0001 C CNN "Part number"
F 5 "160-1446-6-ND" H 5800 4050 50  0001 C CNN " Part Number"
	1    5800 4050
	0    -1   -1   0   
$EndComp
$Comp
L Connector:Conn_01x04_Male J4
U 1 1 5CE27928
P 3550 5000
AR Path="/5CCF6F7E/5CE27928" Ref="J4"  Part="1" 
AR Path="/5D002D0B/5CE27928" Ref="J4"  Part="1" 
AR Path="/5D086332/5CE27928" Ref="J?"  Part="1" 
F 0 "J4" H 3522 4882 50  0000 R CNN
F 1 "Conn_01x04_Male" V 3300 5200 50  0000 R CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x04_P2.54mm_Vertical" H 3550 5000 50  0001 C CNN
F 3 "https://cdn.harwin.com/pdfs/D01-992.pdf" H 3550 5000 50  0001 C CNN
F 4 "952-2521-ND" H 3550 5000 50  0001 C CNN " Part Number"
	1    3550 5000
	-1   0    0    1   
$EndComp
$Comp
L Connector:Conn_01x04_Male J5
U 1 1 5CE2B4D2
P 3900 3500
AR Path="/5CCF6F7E/5CE2B4D2" Ref="J5"  Part="1" 
AR Path="/5D002D0B/5CE2B4D2" Ref="J5"  Part="1" 
AR Path="/5D086332/5CE2B4D2" Ref="J?"  Part="1" 
F 0 "J5" H 3872 3382 50  0000 R CNN
F 1 "Conn_01x04_Male" V 3650 3900 50  0000 R CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x04_P2.54mm_Vertical" H 3900 3500 50  0001 C CNN
F 3 "https://cdn.harwin.com/pdfs/D01-992.pdf" H 3900 3500 50  0001 C CNN
F 4 "952-2521-ND" H 3900 3500 50  0001 C CNN " Part Number"
	1    3900 3500
	-1   0    0    1   
$EndComp
Text HLabel 3700 3300 0    50   Input ~ 10
IO_D8
Text HLabel 3700 3400 0    50   Input ~ 10
IO_D9
Text HLabel 3700 3600 0    50   Input ~ 10
IO_D11
Text HLabel 3350 4800 0    50   Input ~ 10
AIN0
Text HLabel 3350 4900 0    50   Input ~ 10
AIN1
Text HLabel 3350 5000 0    50   Input ~ 10
AIN2
Text HLabel 3350 5100 0    50   Input ~ 10
AIN3
Text HLabel 3700 2100 0    50   Input ~ 10
IO_D1
Text HLabel 3700 2200 0    50   Input ~ 10
IO_D2
Text HLabel 3700 2300 0    50   Input ~ 10
IO_D3
Text HLabel 3700 2400 0    50   Input ~ 10
IO_D4
Text HLabel 3700 2500 0    50   Input ~ 10
IO_D5
Text HLabel 3700 2600 0    50   Input ~ 10
IO_D6
Text HLabel 5800 3350 0    50   Input ~ 10
LED_OUT
Text HLabel 6550 3450 1    50   Input ~ 10
LED_R
Text HLabel 6750 3450 1    50   Input ~ 10
LED_G
Text HLabel 6950 3450 1    50   Input ~ 10
LED_B
$Comp
L Device:R R?
U 1 1 5CE394BD
P 6950 3700
AR Path="/5CCEA06C/5CE394BD" Ref="R?"  Part="1" 
AR Path="/5CCF6F7E/5CE394BD" Ref="R10"  Part="1" 
AR Path="/5D002D0B/5CE394BD" Ref="R4"  Part="1" 
AR Path="/5D086332/5CE394BD" Ref="R?"  Part="1" 
F 0 "R4" V 7050 3700 50  0000 L CNN
F 1 "330" V 7050 3500 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 6880 3700 50  0001 C CNN
F 3 "https://www.rohm.com/datasheet/KTR03EZPF/ktr-e" H 6950 3700 50  0001 C CNN
F 4 "RHM330AZCT-ND" H 6950 3700 50  0001 C CNN " Part Number"
	1    6950 3700
	1    0    0    -1  
$EndComp
$Comp
L Device:R R?
U 1 1 5CE394C3
P 6750 3700
AR Path="/5CCEA06C/5CE394C3" Ref="R?"  Part="1" 
AR Path="/5CCF6F7E/5CE394C3" Ref="R9"  Part="1" 
AR Path="/5D002D0B/5CE394C3" Ref="R3"  Part="1" 
AR Path="/5D086332/5CE394C3" Ref="R?"  Part="1" 
F 0 "R3" V 6650 3700 50  0000 L CNN
F 1 "330" V 6650 3500 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 6680 3700 50  0001 C CNN
F 3 "https://www.rohm.com/datasheet/KTR03EZPF/ktr-e" H 6750 3700 50  0001 C CNN
F 4 "RHM330AZCT-ND" H 6750 3700 50  0001 C CNN " Part Number"
	1    6750 3700
	1    0    0    -1  
$EndComp
$Comp
L Device:R R?
U 1 1 5CE394C9
P 6550 3700
AR Path="/5CCEA06C/5CE394C9" Ref="R?"  Part="1" 
AR Path="/5CCF6F7E/5CE394C9" Ref="R8"  Part="1" 
AR Path="/5D002D0B/5CE394C9" Ref="R2"  Part="1" 
AR Path="/5D086332/5CE394C9" Ref="R?"  Part="1" 
F 0 "R2" V 6450 3700 50  0000 L CNN
F 1 "330" V 6450 3500 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 6480 3700 50  0001 C CNN
F 3 "https://www.rohm.com/datasheet/KTR03EZPF/ktr-e" H 6550 3700 50  0001 C CNN
F 4 "RHM330AZCT-ND" H 6550 3700 50  0001 C CNN " Part Number"
	1    6550 3700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR016
U 1 1 5CE3F7BF
P 5800 4400
AR Path="/5CCF6F7E/5CE3F7BF" Ref="#PWR016"  Part="1" 
AR Path="/5D002D0B/5CE3F7BF" Ref="#PWR05"  Part="1" 
AR Path="/5D086332/5CE3F7BF" Ref="#PWR?"  Part="1" 
F 0 "#PWR05" H 5800 4150 50  0001 C CNN
F 1 "GND" H 5805 4227 50  0000 C CNN
F 2 "" H 5800 4400 50  0001 C CNN
F 3 "" H 5800 4400 50  0001 C CNN
	1    5800 4400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5800 4200 5800 4400
Text HLabel 3700 3500 0    50   Input ~ 10
IO_D10
$Comp
L Connector:Conn_01x10_Male J3
U 1 1 5CE5D9BC
P 3900 2500
AR Path="/5CCF6F7E/5CE5D9BC" Ref="J3"  Part="1" 
AR Path="/5D002D0B/5CE5D9BC" Ref="J3"  Part="1" 
AR Path="/5D086332/5CE5D9BC" Ref="J?"  Part="1" 
F 0 "J3" H 3872 2474 50  0000 R CNN
F 1 "Conn_01x10_Male" V 3650 2800 50  0000 R CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x10_P2.54mm_Vertical" H 3900 2500 50  0001 C CNN
F 3 "https://cdn.harwin.com/pdfs/D01-992.pdf" H 3900 2500 50  0001 C CNN
F 4 "952-2521-ND" H 3900 2500 50  0001 C CNN " Part Number"
	1    3900 2500
	-1   0    0    -1  
$EndComp
Text HLabel 3700 2700 0    50   Input ~ 10
IO_D7
Wire Wire Line
	3600 2900 3700 2900
Wire Wire Line
	3600 2800 3700 2800
$Comp
L power:GND #PWR015
U 1 1 5CE63A36
P 3600 3000
AR Path="/5CCF6F7E/5CE63A36" Ref="#PWR015"  Part="1" 
AR Path="/5D002D0B/5CE63A36" Ref="#PWR04"  Part="1" 
AR Path="/5D086332/5CE63A36" Ref="#PWR?"  Part="1" 
F 0 "#PWR04" H 3600 2750 50  0001 C CNN
F 1 "GND" V 3605 2872 50  0000 R CNN
F 2 "" H 3600 3000 50  0001 C CNN
F 3 "" H 3600 3000 50  0001 C CNN
	1    3600 3000
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR0117
U 1 1 5CF63594
P 3600 2900
AR Path="/5CCF6F7E/5CF63594" Ref="#PWR0117"  Part="1" 
AR Path="/5D002D0B/5CF63594" Ref="#PWR03"  Part="1" 
AR Path="/5D086332/5CF63594" Ref="#PWR?"  Part="1" 
F 0 "#PWR03" H 3600 2750 50  0001 C CNN
F 1 "+5V" V 3615 3028 50  0000 L CNN
F 2 "" H 3600 2900 50  0001 C CNN
F 3 "" H 3600 2900 50  0001 C CNN
	1    3600 2900
	0    -1   -1   0   
$EndComp
$Comp
L power:+3.3V #PWR0118
U 1 1 5CF63D38
P 3600 2800
AR Path="/5CCF6F7E/5CF63D38" Ref="#PWR0118"  Part="1" 
AR Path="/5D002D0B/5CF63D38" Ref="#PWR02"  Part="1" 
AR Path="/5D086332/5CF63D38" Ref="#PWR?"  Part="1" 
F 0 "#PWR02" H 3600 2650 50  0001 C CNN
F 1 "+3.3V" V 3615 2928 50  0000 L CNN
F 2 "" H 3600 2800 50  0001 C CNN
F 3 "" H 3600 2800 50  0001 C CNN
	1    3600 2800
	0    -1   -1   0   
$EndComp
Wire Wire Line
	3600 3000 3700 3000
Wire Wire Line
	5800 3350 5800 3500
Wire Wire Line
	8500 4200 8500 4050
Wire Wire Line
	8500 4550 8500 4500
$Comp
L Device:R R11
U 1 1 5CE41EFC
P 8500 4350
AR Path="/5CCF6F7E/5CE41EFC" Ref="R11"  Part="1" 
AR Path="/5D002D0B/5CE41EFC" Ref="R5"  Part="1" 
AR Path="/5D086332/5CE41EFC" Ref="R?"  Part="1" 
F 0 "R5" H 8570 4396 50  0000 L CNN
F 1 "1K" H 8570 4305 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" V 8430 4350 50  0001 C CNN
F 3 "https://industrial.panasonic.com/cdbs/www-data/pdf/RDA0000/AOA0000C304.pdf" H 8500 4350 50  0001 C CNN
F 4 "P1.00KHDKR-ND" H 8500 4350 50  0001 C CNN " Part Number"
	1    8500 4350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR018
U 1 1 5CE41733
P 8500 4550
AR Path="/5CCF6F7E/5CE41733" Ref="#PWR018"  Part="1" 
AR Path="/5D002D0B/5CE41733" Ref="#PWR08"  Part="1" 
AR Path="/5D086332/5CE41733" Ref="#PWR?"  Part="1" 
F 0 "#PWR08" H 8500 4300 50  0001 C CNN
F 1 "GND" H 8505 4377 50  0000 C CNN
F 2 "" H 8500 4550 50  0001 C CNN
F 3 "" H 8500 4550 50  0001 C CNN
	1    8500 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	8800 3900 8650 3900
Text HLabel 8800 3900 2    50   Input ~ 10
POT
$Comp
L Device:R_POT RV1
U 1 1 5CE29D0F
P 8500 3900
AR Path="/5CCF6F7E/5CE29D0F" Ref="RV1"  Part="1" 
AR Path="/5D002D0B/5CE29D0F" Ref="RV1"  Part="1" 
AR Path="/5D086332/5CE29D0F" Ref="RV?"  Part="1" 
F 0 "RV1" H 8431 3946 50  0000 R CNN
F 1 "10K" H 8431 3855 50  0000 R CNN
F 2 "Potentiometer_SMD:Potentiometer_ACP_CA6-VSMD_Vertical" H 8500 3900 50  0001 C CNN
F 3 "https://www.ttelectronics.com/TTElectronics/media/ProductFiles/Potentiometers/Datasheets/P160.pdf" H 8500 3900 50  0001 C CNN
F 4 "987-1305-ND" H 8500 3900 50  0001 C CNN " Part Number"
	1    8500 3900
	1    0    0    -1  
$EndComp
Wire Wire Line
	6550 3450 6550 3550
Wire Wire Line
	6750 3550 6750 3450
Wire Wire Line
	6950 3550 6950 3450
$Comp
L Device:LED_CRGB D5
U 1 1 5CF74075
P 6750 4250
F 0 "D5" V 6796 3920 50  0000 R CNN
F 1 "LED_CRGB" V 6705 3920 50  0000 R CNN
F 2 "LED_SMD:LED_RGB_1210" H 6750 4200 50  0001 C CNN
F 3 "https://docs.broadcom.com/docs/AV02-0610EN" H 6750 4200 50  0001 C CNN
F 4 "	516-1778-6-ND" H 6750 4250 50  0001 C CNN " Part Number"
	1    6750 4250
	0    -1   -1   0   
$EndComp
Wire Wire Line
	6550 3850 6550 4050
Wire Wire Line
	6750 3850 6750 4050
Wire Wire Line
	6950 3850 6950 4050
Text Notes 3350 1900 0    79   ~ 16
Digital Input 
Text Notes 3750 4379 2    79   ~ 16
Analog Input 
Wire Notes Line width 20
	4350 1950 3000 1950
Wire Notes Line width 20
	4350 4050 3000 4050
Wire Notes Line width 20
	4350 1950 4350 4050
Wire Notes Line width 20
	3000 1950 3000 4050
Wire Notes Line
	3000 4650 3000 4450
Wire Notes Line width 20
	3000 4450 3900 4450
Wire Notes Line width 20
	3900 4450 3900 5300
Wire Notes Line width 20
	3900 5300 3000 5300
Wire Notes Line width 20
	3000 5300 3000 4450
Text Notes 5750 2900 0    79   ~ 16
Indicadres Leds
Text Notes 7900 2950 0    79   ~ 16
Potenciometro A_IN
$Comp
L power:GND #PWR?
U 1 1 5D01E3C1
P 6750 4850
AR Path="/5CCF6F7E/5D01E3C1" Ref="#PWR?"  Part="1" 
AR Path="/5D002D0B/5D01E3C1" Ref="#PWR0110"  Part="1" 
AR Path="/5D086332/5D01E3C1" Ref="#PWR?"  Part="1" 
F 0 "#PWR0110" H 6750 4600 50  0001 C CNN
F 1 "GND" H 6755 4677 50  0000 C CNN
F 2 "" H 6750 4850 50  0001 C CNN
F 3 "" H 6750 4850 50  0001 C CNN
	1    6750 4850
	1    0    0    -1  
$EndComp
Wire Wire Line
	6750 4450 6750 4850
Wire Wire Line
	8500 3450 8500 3750
$Comp
L power:VDD #PWR0119
U 1 1 5CF66350
P 8500 3450
AR Path="/5CCF6F7E/5CF66350" Ref="#PWR0119"  Part="1" 
AR Path="/5D002D0B/5CF66350" Ref="#PWR07"  Part="1" 
AR Path="/5D086332/5CF66350" Ref="#PWR?"  Part="1" 
F 0 "#PWR07" H 8500 3300 50  0001 C CNN
F 1 "VDD" H 8517 3623 50  0000 C CNN
F 2 "" H 8500 3450 50  0001 C CNN
F 3 "" H 8500 3450 50  0001 C CNN
	1    8500 3450
	1    0    0    -1  
$EndComp
Text Notes 4700 1200 0    79   Italic 0
Funcionamiento:\nIndicadores leds y Led RGB para interacción del usuario\nPines de entradas analógicas y de entrada /salida digitales\nAsi como un potenciometro para  leer entrada analógica.
Wire Notes Line width 20
	7550 3050 7550 5150
Wire Notes Line width 20
	7550 5150 5350 5150
Wire Notes Line width 20
	5350 5150 5350 3050
Wire Notes Line width 20
	5350 3050 7550 3050
$EndSCHEMATC
